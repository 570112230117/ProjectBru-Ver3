$(document).ready(function () {
	...
	$('#appliances').change(function () {
		...
		var testAjexBean = { "appliances": $('#appliances').val() };
		$.ajax({
			type: "POST",
			url: "/product",
	...
	$('#product').change(function () {
		$('#brand').empty();
		var testAjexBean = { "appliances": $('#appliances').val(), "brand": $('#product').val() };
		$.ajax({
			type: "POST",
			url: "/brand",
			data: JSON.stringify(testAjexBean),
			...
			success: function (msg) {
				$('#brand').append('<option value="">== กรุณาเลือก ==</option>');
				for (var i = 0; i < msg.length; i++) {
					$('#brand').append('<option value="' + msg[i].productId + '">' + msg[i].name + '</option>');
				}
			}
		});
	});
